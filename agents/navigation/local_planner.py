...
class LocalPlanner(object):
    ...
    # MIN_DISTANCE_PERCENTAGE = 0.7 # NOTE best val so far
    MIN_DISTANCE_PERCENTAGE = 0.5

    def __init__(self, vehicle, opt_dict=None, recording=False):
        """
    ...
    def _init_controller(self, opt_dict):
        ...
        """
        # default params
        self._dt = 1.0 / 10.0 # 1.0 / 20.0
        self._target_speed = 20.0  # Km/h
        self._sampling_radius = self._target_speed * 1 / 3.6  # 1 seconds horizon
        self._min_distance = self._sampling_radius * self.MIN_DISTANCE_PERCENTAGE
        args_lateral_dict = {
            # 'K_P': 1.95,
            # 'K_P': 1.75, # NOTE good value so far
            'K_P': 1.65,
            'K_D': 0.01,
            'K_I': 1.4,
            'dt': self._dt}
    ...
    def run_step(self, debug=False):
        ...
        if max_index >= 0:
            for i in range(max_index + 1):
                if self.recording: # save already reached waypoints in trajectory when recording
                    self._waypoint_history.append(self._waypoint_buffer.popleft())
                else:
