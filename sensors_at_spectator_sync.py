...
def main():
    ...
    try:
        ...
        # NOTE for more information see about limitations see: https://github.com/carla-simulator/carla/issues/515
        ...
        if not args.left_rel_location:
            cam_rel_transform_l = carla.Transform(carla.Location(x=0.0, y=-args.baseline/2.0, z=0.0))
            args.left_rel_location = [0.0, -args.baseline/2.0, 0.0]
        else:
            pos_x, pos_y, pos_z = args.left_rel_location
        ...
        for sensor in args.sensors:
            sensor_actor, sensor_id = spawn_camera_sensor(
                sensor, world, args.sensor_width, args.sensor_height, args.fov, spec, cam_rel_transform_l, args.baseline, base_path
            )
